...
export default function ParticlesBackground() {
  ...
  const updateConnections = useCallback((particles: Particle[], canvasHeight: number) => {
    ...
    for (let i = 0; i < particles.length; i++) {
      ...
      for (let j = i + 1; j < particles.length; j++) {
        ...
        if (p2DrawY < -connectionDistance * 2 || p2DrawY > canvasHeight + connectionDistance * 2) {
          continue;
        }
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      scrollOffsetRef.current += (targetOffset - scrollOffsetRef.current) * 0.08;
      const scrollOffset = scrollOffsetRef.current;
