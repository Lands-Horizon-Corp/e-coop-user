...
import { MoneyBagIcon, RenderIcon } from '@/components/icons'
...
import { useAuthUserWithOrgMemberProfile } from '../auth/authgentication.store'
import { GeneralLedgerEntriesModal } from '../general-ledger/components/general-ledger-entries'
import { CoopBackground } from '../home/components/coop-bg'
import { IMemberAccountingLedger } from '../member-account-ledger'
import { useGetMemberAccountingLedger } from '../member-accounting-ledger/member-accounting-ledger.service'
import WalletBalance from './components/member-wallet'
...
export default function DashboardPage() {
    ...
    const { data = [] } = useGetMemberAccountingLedger({
        memberProfileId: member_profile.id,
    })
    ...
    return (
        <div className="flex flex-col min-h-screen">
            ...
            <QrCodeModal
                ...
                qrData={qrData}
            />
            <section className="relative bg-linear-to-br from-primary/5 via-background to-accent/5 py-12 overflow-hidden">
                <div className="container mx-auto px-4 md:px-6">
                    <div className="max-w-3xl">
                        ...
                        <h1 className="text-4xl md:text-5xl font-bold mb-4 bg-linear-to-br from-foreground to-foreground/70 bg-clip-text text-transparent">
                            Welcome Back,{' '}
                            {member_profile?.first_name || 'Member'}
                        </h1>
            ...
            <div className="container mx-auto px-4 md:px-6 py-8 ">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
                    ...
                    <div className="lg:col-span-4 space-y-8">
                        {/* Wallet Card */}
                        {member_profile && (
                            <WalletBalance memberId={member_profile?.id} />
                        )}
                            ...
                            <div className="flex items-center justify-between mb-6">
                                <div>
                                    ...
                                    <p className="text-sm text-muted-foreground">
                                        Manage/See your deposit accounts
                                    </p>
                                </div>
                            </div>
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                {data.map((accountingLedger) => (
                                    <DepositAccountCard
                                        accountingLedger={accountingLedger}
                                        key={accountingLedger.id}
                                    />
                                ))}

                                {data.length === 0 && (
                                    <div className="mx-auto mt-16 col-span-full flex flex-col items-center text-center">
                                        ...
                                        <p className="mt-1 text-sm text-muted-foreground">
                                            You don't have any deposit accounts
                                            at the moment. Once you open a
                                            deposit account, it will appear
                                            here.
                                        </p>
                                    </div>
                                )}
                        ...
                        <div className="">
                            ...
                            {activeLoans.map((loan) => (
                                <Card
                                    className="mb-4 bg-primary/20 border hover:shadow-lg dark:border-background/50 rounded-xl transition-all duration-300 relative overflow-hidden"
                                    key={loan.id}
                                >
                        ...
                        <div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                {upcomingEvents.map((event) => {
                                    ...
                                    return (
                                        <Card
                                            className="hover:shadow-xl hover:-translate-y-1 transition-all duration-300 cursor-pointer group dark:border-background bg-muted overflow-hidden"
                                            key={event.id}
                                        >
                ...
                {/* KPI Cards Row */}
                <div className="grid rid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8 mt-8">
                    {/* Total Portfolio Card */}
                    <Card className="bg-background backdrop-blur-xl dark:border-background shadow-2xl hover:shadow-blue-500/20 transition-all duration-300 relative overflow-hidden group">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
                    ...
                    <Card className="lg:col-span-2 bg-muted dark:bg-primary/20 backdrop-blur-xl border-background shadow-2xl relative overflow-hidden">
                    ...
                    <Card className="bg-background dark:bg-background backdrop-blur-xl border-background shadow-2xl relative overflow-hidden">
                        ...
                        <div className="space-y-6">
                            ...
                            <div className="space-y-3">
                                ...
                                {selectedLoan.paymentHistory.map((payment) => (
                                    <Card
                                        className="hover:shadow-md bg-muted transition-shadow"
                                        key={payment.id}
                                    >
                                        <CardContent className="p-2.5">
                                            <div className="flex items-center justify-between">
...
const DepositAccountCard = ({
    accountingLedger,
}: {
    accountingLedger: IMemberAccountingLedger
}) => {
    const Icon = accountingLedger.account.icon
    const AccountModalState = useModalState()

    return (
        <>
            <GeneralLedgerEntriesModal
                {...AccountModalState}
                generalLedgerEntriesProps={{
                    memberAccountingLedgerId: accountingLedger.id,
                    focusedAccount: accountingLedger.account,
                }}
            />
            <Card
                className="dark:bg-background bg-primary/90 backdrop-blur-xl dark:border-background shadow-2xl hover:shadow-blue-500/20 transition-all duration-300 relative overflow-hidden group"
                key={accountingLedger.id}
                onClick={() => {
                    AccountModalState.onOpenChange(true)
                }}
            >
                ...
                <CardHeader className="flex flex-row items-center justify-between pb-3">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <CardTitle className="text-lg dark:muted text-white">
                                {accountingLedger.account.name}
                            </CardTitle>
                            <p className="text-xs text-muted dark:text-muted-foreground">
                                Last deposit:{' '}
                                {accountingLedger.updated_at &&
                                    toReadableDateTime(
                                        accountingLedger.updated_at
                                    )}
                            </p>
                    ...
                    <Button
                        ...
                        variant="ghost"
                    >
                        <QrCode className="size-5 text-white dark:text-muted-foreground" />
                    </Button>
                </CardHeader>
                <CardContent>
                    <div className="text-2xl font-bold text-secondary dark:text-primary">
                        ₱{accountingLedger.balance.toLocaleString()}
                    </div>
                </CardContent>
            </Card>
        </>
