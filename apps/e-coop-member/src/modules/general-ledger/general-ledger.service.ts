...
import { IMemberAccountingLedgerTotal } from '../member-account-ledger'
import {
    IGeneralLedger,
...
// API function to get general ledger by member accounting ledger ID
export const getGeneralLedgerByMemberAccountingLedgerId = async (
    memberAccountingLedgerId: TEntityId
): Promise<IGeneralLedger[]> => {
    const response = await API.get<IGeneralLedger[]>(
        `${generalLedgerAPIRoute}/member-accounting-ledger/${memberAccountingLedgerId}`
    )
    return response.data
}

export const useGetGeneralLedgerByMemberAccountingLedgerId = ({
    memberAccountingLedgerId,
    options,
}: {
    memberAccountingLedgerId?: TEntityId
    ...
    return useQuery<IGeneralLedger[], Error>({
        ...options,
        enabled: Boolean(memberAccountingLedgerId),
        queryKey: [
            generalLedgerBaseKey,
            'member-accounting-ledger',
            memberAccountingLedgerId,
        ...
        queryFn: async () =>
            getGeneralLedgerByMemberAccountingLedgerId(
                memberAccountingLedgerId as TEntityId
            ),
    })
}
// API function to get total of member accounting ledger
export const getMemberAccountingLedgerTotal = async (
    ...
    const response = await API.get<IMemberAccountingLedgerTotal>(
        `${generalLedgerAPIRoute}/member-accounting-ledger/${memberAccountingLedgerId}/total`
    )
    return response.data
}

export const useGetMemberAccountingLedgerTotal = ({
    memberAccountingLedgerId,
    options,
}: {
    memberAccountingLedgerId?: TEntityId
