...
// USE only kapag sure na user, organization, exist in user auth store, pero not sure if may branch
export const useAuthUserWithOrgMemberProfile = <TUser = IUserBase>() => {
    const { currentAuth, ...rest } = useAuthUserWithOrgBranch<TUser>()

    if (!currentAuth.member_profile) {
        throw new Error('Authenticated user has no member profile context.')
    }

    return {
        ...rest,
        currentAuth: currentAuth as typeof currentAuth & {
            member_profile: NonNullable<
                IUserOrganization & { member_profile: IMemberProfile }
            >
        },
    }
}
