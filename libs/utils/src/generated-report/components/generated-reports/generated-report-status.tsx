...
const generatedReportStatusVariants = cva(
    'inline-flex items-center rounded-full border px-2.5 py-0.5 text-xs font-semibold transition-colors focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2',
    {
        variants: {
            variant: {
                ...
                failed: 'bg-destructive/40 text-destructive/70 border-red-200 hover:bg-destructive/70 dark:bg-destructive/40/20 dark:text-destructive dark:border-red-800 dark:hover:bg-destructive/40/30',
                unknown:
...
const GeneratedReportStatusBadge = forwardRef<HTMLDivElement, Props>(
    ({ status, className }, ref) => {
        ...
        const variant: BadgeVariant = isValidStatus
            ? (status as BadgeVariant)
            : 'unknown'
        const displayText = isValidStatus ? status : 'unknown'
        ...
        return (
            ...
            >
                {displayText}
                {variant === 'completed' ? (
                    <CheckIcon className="text-primary ml-1" />
                ) : (
                    ''
                )}
            </div>
...
GeneratedReportStatusBadge.displayName = 'GeneratedReportStatusBadge'

export default GeneratedReportStatusBadge
