...
const MemberDepartmentPage = () => {
    ...
    useSubscribe('member_department', `create.branch.${branch_id}`, () => {
        queryClient.invalidateQueries({
            queryKey: ['member-department', 'paginated'],
        })
    })

    useSubscribe('member_department', `update.branch.${branch_id}`, () => {
        queryClient.invalidateQueries({
            queryKey: ['member-department', 'paginated'],
        })
    })

    useSubscribe('member_department', `delete.branch.${branch_id}`, () => {
        queryClient.invalidateQueries({
            queryKey: ['member-department', 'paginated'],
        })
    })
