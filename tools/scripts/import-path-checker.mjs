...
export async function collectSourceFiles(rootDir, extensions = DEFAULT_EXTENSIONS) {
  const files = [];
  const stack = [rootDir];
...
export function findBadModuleImports({ content, moduleNames, filePath, modulesDir }) {
  const matches = [];
  ...
  const moduleRoot = currentModuleName && modulesDir
  ...
  for (const match of matches) {
    ...
    if (moduleRoot && !isRelativeOutsideModule({
      specifier: normalized,
      filePath,
      moduleRoot,
    })) {
...
export async function checkModuleFolder({ moduleDir, moduleNames, modulesDir }) {
...
export function applyModuleImportFixes({ content, issues }) {
  ...
  for (const issue of issues) {
    ...
    const nextContent = updatedContent.replace(replaceRegex, `$1${expected}$1`);
...
function isRelativeToSiblingModule(specifier, moduleNames, currentModuleName) {
  ...
  let index = 0;
  while (index < parts.length && (parts[index] === ".." || parts[index] === "...")) {
    index += 1;
  }
...
function matchModuleName({ specifier, moduleNames, filePath, modulesDir }) {
  for (const name of moduleNames) {
    if (specifier.includes(`/modules/${name}`) || specifier.includes(`libs/modules/${name}`)) {
      return name;
    }
  }
