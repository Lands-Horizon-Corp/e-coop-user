...
import { existsSync, mkdirSync, readdirSync, renameSync, rmSync, rmdirSync, statSync } from 'node:fs';
import path from 'node:path';

const folderName = process.argv[2]?.trim();

if (!folderName) {
  console.error('Missing module folder name. Usage: bun run g:modules <folder_name>');
  process.exit(1);
}
...
const legacyModuleRoot = path.join(workspaceRoot, 'libs', 'modules', 'src', folderName);
...
function moveEntry(sourcePath, destinationPath) {
  try {
    ...
    if (error && error.code === 'EXDEV') {
      throw new Error(`Cross-device move is not supported for ${sourcePath}`);
    }
...
function migrateLegacyModule() {
  ...
  if (!statSync(legacyModuleRoot).isDirectory()) {
    throw new Error(`Legacy module path is not a directory: ${legacyModuleRoot}`);
  }
  ...
  for (const entry of legacyEntries) {
    ...
    if (existsSync(destinationPath)) {
      if (entry.isDirectory() && statSync(destinationPath).isDirectory()) {
        mergeDirectory(sourcePath, destinationPath);
        continue;
      }

      throw new Error(`Cannot move ${sourcePath}. Destination exists: ${destinationPath}`);
    }
