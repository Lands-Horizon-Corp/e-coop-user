...
const DEFAULT_EXTENSIONS = new Set([".ts", ".tsx", ".js", ".jsx", ".mts", ".cts"]);
const DEFAULT_SKIP_DIRS = new Set([".git", ".nx", "dist", "build", "coverage", "node_modules"]);
...
function convertDefaultExport(content) {
  ...
  const match = lastLine.match(/^\s*export\s+default\s+([A-Za-z0-9_]+)\s*;?\s*$/);
  if (!match) {
    return { updated: false, content };
  }
